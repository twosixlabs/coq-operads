...
(*returns the ith element of a list of types*)
(*if it is out of range, it returns unit*) 
Definition lookupT (i: nat)(xs: list Type) : Type := 
  @lookup Type unit i xs .
...
Record Operad: Type :=
{
  interp : forall (d : Type)(c : list Type), Type;
}.

Notation "operad 〘 d , c 〙" := (interp operad d c)  (at level 10).

(* ---------------------------------- *)
(* List equalitiy Theorems for type casting in the definition of an operads *)
...
Definition unityLCast {operad : Operad}{d : Type} {c : list Type}
(e : (operad〘d,insert 0 [d] c〙)) : ( operad〘d,c〙). 
Proof. 
...
Lemma unityRInsert: forall {def: Type}(c: list Type)(ci: Type)(n i: nat),
 1 <= n -> i < n -> length c = n ->  @lookup Type def i c = ci -> insert i c [ci] = c.
Proof.
  intros def c ci. 
...
Definition unityRCast {operad : Operad}{d ci:Type}{c: list Type}(n i: nat)(p: lookupT i c = ci)(q0: 1 <= n)
  (q1: i < n)(r: length c = n)(e : (operad 〘d, insert i c [ci]〙)) : (operad 〘d, c〙).
Proof. 
  intros. 
  rewrite (@unityRInsert unit c ci n i q0 q1 r p) in e. apply e. 
Defined.  
...
Definition vertCast {operad : Operad} {n m i j : nat}
          ...
          (e : (operad 〘d, insert (i+j) (insert i  c b) a〙)) : 
          (operad 〘d, insert i c (insert j b a)〙). 
Proof.
...
Definition horizCast  {operad : Operad} (n m l i j : nat)(a b c : list Type)(d:Type)(p0: 2 <= n)(p1 : 1 <= m)(p2: 1 <= l)
  (q0: i < j)(q1: j < n)(r0: length c = n)(r1: length a = l)(r2 : length b = m)
  (e:  (operad〘d, insert (l-1 + j) (insert i c a) b〙)) :  (operad 〘d, insert i (insert j c b) a〙). 
Proof.
  rewrite (@horizInsert Type n m l i j a b c p0 p1 p2 q0 q1 r0 r1 r2) in e. apply e. 
Defined. 

(*In order to finish our formal definition of colored operads, 
we need to define the laws that an operad must follow. 
We first include several lemmas which will be referenced in formulating those laws. *)
...
Lemma n_gt_ineq {i n : nat} (p0 : i < n) : 1 <= n.
Proof.
sauto.
Qed.
...
Lemma vertLookup : forall {n m i j : nat} {bj : Type}
          {b c : list Type}
          (q1 : lookupT j b = bj)
          (r0: i < n)(r1: j < m) 
...
Lemma horizLookupL : forall {n i j : nat} {ci : Type} {b c : list Type} (p0: i < j) (p1 : j < n) (r0 : lookupT i c = ci) (s0: length c = n),
...
Lemma horizLookupR : forall {n l i j : nat} {cj : Type} {a c : list Type} (p0 : i < j) (p1 : j < n) (q2 : 1 <= l) (r1 : lookupT j c = cj) (s0 : length c = n) (s2 : length a = l), 
...
(* This record defines the operad laws. *)
Record operadLaws (operad : Operad) : Type :=
{
  opId: forall (d : Type), (operad 〘d, [d]〙)

; operadComp : forall (i n : nat) (d ci : Type) (c b : list Type) 
    (r : length c = n) (n1 : 1 <= n) (il : i < n), 
    lookupT i c= ci -> (operad 〘d,c〙) * (operad 〘ci,b〙) -> (operad 〘d,insert i c b〙)

; unityL: forall (d : Type) (c : list Type) (e : (operad〘d,c〙)),
   unityLCast(operadComp 0 1 d d [d] c eq_refl (le_n 1) PeanoNat.Nat.lt_0_1 eq_refl (opId d, e)) = e 

; unityR : forall (n i : nat) (d ci : Type) (c : list Type ) (p : lookupT i c = ci)(q0: 1 <= n)(q1: i < n) 
              (r: length c = n) (e : (operad〘d,c〙)), 
            @unityRCast operad d ci c n i p q0 q1 r (operadComp i n d ci c (ci::nil) r q0 q1 p (e, opId ci)) = e

; vertAssoc: forall (n m l i j : nat) (d ci bj : Type)
          (a b c : list Type) 
          (p0: 1 <= n )(p1 : 1 <= m) (p2 : 1 <= l)
          (q0 : lookupT i c = ci) (q1 : lookupT j b = bj)
          (r0: i < n)(r1: j < m) 
          (s0: length c = n)(s1: length b = m)(s2: length a = l)
          (e : (operad 〘d,c〙)) (f : (operad 〘ci, b〙)) (g : (operad 〘bj, a〙)), 
...
; horizAssoc: forall (n m l i j : nat) (d ci cj : Type) 
          (a b c : list Type ) 
          (p0 : i < j)(p1 : j < n)
          (q0: 2 <= n)(q1: 1 <= m)(q2: 1 <= l) 
          (r0 : lookupT i c = ci) (r1 : lookupT j c = cj)
          (s0: length c = n)(s1: length b = m)(s2: length a = l)
          (e : (operad〘d,c〙)) (f : (operad〘ci,a〙)) (g : (operad 〘cj,b〙)),
...
; perm: forall (c c': list Type)(d : Type)
   (p: 1 <= length c ),
  Permutation c c' -> iso (operad〘d, c〙) (operad〘d, c'〙)
}.
